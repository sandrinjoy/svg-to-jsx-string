// credits:
// https://github.com/crispinanderson/react-external-svg/blob/master/src/utils.js
...
const processAttributeName = function (name) {
  return DATA_ATTRIBUTE.test(name) ? name : camelCase(name);
};
...
const sanitizeAttributes = function (attributes) {
  ...
  Object.keys(attributes).forEach(function (name) {
    var newName = processAttributeName(name);
    if (newName !== name) {
  ...
  const allowed = ALLOWED_ATTRIBUTES.reduce(function (hash, name) {
    if (name in attributes) {
      var unnamespacedName = unnamespaceAttributeName(name);
...
const styleAttribute = function (string) {
  var object = string.split(/\s*;\s*/g).reduce(function (hash, keyValue) {
    ...
    var key = camelCase((split[0] || "").trim());
...
export { sanitizeAttributes, noUnSupportedTagNames };
